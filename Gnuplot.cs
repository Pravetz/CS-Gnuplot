...
namespace GnuplotBindUtil
{
	public class Gnuplot
	{
		...
		public void plot()
		{
			...
			gnuplot.StartInfo.RedirectStandardError = true;
			gnuplot.StartInfo.CreateNoWindow = true;
			gnuplot.Start();
			
			using (StreamWriter gnuci = gnuplot.StandardInput)
			{
				...
				for (int i = 0; i < line_colors.Count; i++)
				{
					plot_args += $"index {i} title '{line_names[i]}' with {line_plot_types[i]} linestyle {i + 1}";
				...
				gnuci.WriteLine($"plot {plot_args}");
			...
			// Read errors
			string errors = gnuplot.StandardError.ReadToEnd();
	
			if (!string.IsNullOrWhiteSpace(errors))
			{
				throw new Exception("Gnuplot error:\n" + errors);
			}
		}
